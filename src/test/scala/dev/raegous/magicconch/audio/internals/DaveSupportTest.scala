...
class DaveSupportTest extends FunSuite {
  ...
  test("ULEB128 should round-trip protocol nonce values") {
    ...
    values.foreach { value =>
      ...
      assertEquals(
        readUleb128(encoded, 0, encoded.length),
        Right((value, encoded.length))
      )
    }
  }
  ...
  test("GatewayBinaryCodec should round-trip external sender packages") {
    val message = MlsExternalSenderPackage(
      ...
      externalSender = ExternalSender(
        ...
        credential =
          Credential(credentialType = 1, identity = Array[Byte](5, 6, 7))
      )
    )
    ...
    GatewayBinaryCodec.decode(GatewayBinaryCodec.encode(message)) match {
      case Right(decoded: MlsExternalSenderPackage) =>
        ...
        assert(
          decoded.externalSender.signatureKey.sameElements(
            message.externalSender.signatureKey
          )
        )
        assertEquals(decoded.externalSender.credential.credentialType, 1)
        assert(
          decoded.externalSender.credential.identity
            .sameElements(message.externalSender.credential.identity)
        )
      case other =>
  ...
  test("DaveProtocol should build binary MLS key package messages") {
    val payload = (for {
      ...
      keyPackage <- DaveProtocol.buildKeyPackageMessage[IO](
        state,
        "1090123456789012345"
      )
    } yield GatewayBinaryCodec.encode(MlsKeyPackage(keyPackage)))
      .unsafeRunSync()
    ...
    assert(
      MlsMessages
        .parseMlsMessage(payload.drop(1))
        .toOption
        .forall(_.wireFormat != MlsMessages.WireFormatKeyPackage)
    )
  ...
  test("MediaFrameCodec should add and detect DAVE protocol frame footer") {
    ...
    val encrypted = MediaFrameCodec.encryptAudioFrame(
      frame,
      truncatedNonce = 3,
      staticRatchet
    )
  ...
  test(
    "MediaFrameCodec should preserve frame footer metadata across nonce encodings"
  ) {
    ...
    nonces.foreach { nonce =>
      val encrypted =
        MediaFrameCodec.encryptAudioFrame(frame, nonce, staticRatchet)

      assert(MediaFrameCodec.isProtocolFrame(encrypted))
      assertEquals(
        MediaFrameCodec.parse(encrypted).map(_.footer.truncatedNonce),
        Right(nonce)
      )
      assertEquals(
        MediaFrameCodec.parse(encrypted).map(_.interleavedFrame.length),
        Right(frame.length)
      )
    }
  }
  ...
  test(
    "DAVE full nonce should encode truncated nonce in little-endian tail bytes"
  ) {
    ...
    assertEquals(
      nonce.drop(8).toList,
      List(0x04.toByte, 0x03.toByte, 0x02.toByte, 0x01.toByte)
    )
  }

  test(
    "MediaFrameCodec should encode the DAVE frame nonce in the supplemental footer"
  ) {
  ...
  test(
    "MediaFrameCodec should decrypt DAVE media frames and expose footer diagnostics"
  ) {
    val frame = Array.tabulate[Byte](96)(i => ((i * 17) & 0xff).toByte)
    val ratchet = new HkdfSenderKeyRatchet(Array.fill[Byte](32)(0x23.toByte))
    val encrypted = MediaFrameCodec.encryptAudioFrame(
      frame,
      truncatedNonce = 0x01020304L,
      ratchet
    )

    assertEquals(
      MediaFrameCodec.decryptFrame(encrypted, ratchet).map(_.toList),
      Right(frame.toList)
    )
    assertEquals(
      MediaFrameCodec
        .diagnostics(frame.length, encrypted)
        .map(_.truncatedNonce),
      Right(0x01020304L)
    )
    assertEquals(
      MediaFrameCodec.diagnostics(frame.length, encrypted).map(_.generation),
      Right(1)
    )
  }

  test(
    "HkdfSenderKeyRatchet should derive stable generation keys from nonce MSB"
  ) {
    ...
    val expectedGeneration0 = MlsPrimitives.expandWithLabel(
      baseSecret,
      "key",
      Array[Byte](0, 0, 0, 0),
      16
