...
class VoiceProtocolTest extends FunSuite {
  ...
  test("Nonce should encode counters in big-endian byte order") {
    ...
    assertEquals(Nonce.forAES256GCM(counter).take(4).toList, List[Byte](1, 2, 3, 4))
    assertEquals(Nonce.forXChaCha20(counter).take(4).toList, List[Byte](1, 2, 3, 4))
  }

  test("RtpTransportState.initial should start sequence and timestamp at zero with nonce 1") {
    assertEquals(
  ...
  test("RTPPacket should append the nonce suffix after payload bytes") {
    ...
    assertEquals(bytes.slice(12, 14).toList, List[Byte](0x55.toByte, 0x66.toByte))
    assertEquals(bytes.takeRight(4).toList, List[Byte](1, 2, 3, 4))
  }

  test("RTPPacket AES-GCM payload should decrypt with header AAD and appended nonce") {
    ...
    encryptCipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), new GCMParameterSpec(128, Nonce.forAES256GCM(nonceCounter)))
    ...
    val appendedNonce = packetBytes.takeRight(4).foldLeft(0L)((acc, byte) => (acc << 8) | (byte & 0xFF).toLong)
    val encryptedFromPacket = packetBytes.slice(headerBytes.length, packetBytes.length - 4)

    val decryptCipher = Cipher.getInstance("AES/GCM/NoPadding")
    decryptCipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key, "AES"), new GCMParameterSpec(128, Nonce.forAES256GCM(appendedNonce)))
    decryptCipher.updateAAD(packetBytes.take(headerBytes.length))

    assertEquals(appendedNonce, nonceCounter)
    assertEquals(decryptCipher.doFinal(encryptedFromPacket).toList, plaintext.toList)
  }

  test("RTPPacket XChaCha20-Poly1305 payload should decrypt with header AAD and appended nonce") {
    val header = RTPHeader(sequence = 7, timestamp = 6720L, ssrc = 0x11223344L)
    val headerBytes = RTPHeader.toBytes(header)
    val key = Array.tabulate[Byte](32)(i => (i + 1).toByte)
    val nonceCounter = 1L
  ...
  test("RtpTransportState should progress sequence timestamp and nonce monotonically") {
    val current = RtpTransportState(sequence = 41, timestamp = 1920L, nonceCounter = 9L)
  ...
  test("RtpTransportState should wrap sequence timestamp and nonce at protocol widths") {
