    ...
    for {
      ...
      voiceManager <- Resource.eval {
        for {
          ...
          userVoiceStatesRef <- Ref.of[F, Map[String, Option[String]]](
            Map.empty
          )
          searchResultsRef <- Ref.of[F, Map[String, List[YouTubeSearchResult]]](
            Map.empty
          )
          activeVoiceConnections <- Ref
            .of[F, Map[String, sttp.ws.WebSocket[F]]](Map.empty)
          gatewayWebSocketRef <- Ref.of[F, Option[sttp.ws.WebSocket[F]]](None)
          pendingVoiceConnections <- Ref
            .of[F, Map[String, PendingVoiceConnection]](Map.empty)
          pendingConnectionPromises <- Ref
            .of[F, Map[String, Deferred[F, Either[VoiceError, Unit]]]](
              Map.empty
            )
          activePlaybackFibers <- Ref
            .of[F, Map[String, Fiber[F, Throwable, Unit]]](Map.empty)
...
class VoiceManager[F[_]: Async: fs2.io.process.Processes] private (
    ...
    private val searchResultsRef: Ref[F, Map[String, List[
      YouTubeSearchResult
    ]]],
    private val activeVoiceConnections: Ref[
      F,
      Map[String, sttp.ws.WebSocket[F]]
    ],
    private val gatewayWebSocketRef: Ref[F, Option[sttp.ws.WebSocket[F]]],
    private val pendingVoiceConnections: Ref[
      F,
      Map[String, PendingVoiceConnection]
    ],
    private val pendingConnectionPromises: Ref[
      F,
      Map[String, Deferred[F, Either[VoiceError, Unit]]]
    ],
    private val activePlaybackFibers: Ref[
      F,
      Map[String, Fiber[F, Throwable, Unit]]
    ],
    backend: sttp.client4.WebSocketStreamBackend[F, ?],
  ...
  private val connectingVoiceConnectionGuilds =
  ...
  private val audioSourceManager = new AudioSourceManager[F](
    List(
  ...
  def handleVoiceStateUpdate(voiceState: VoiceStateUpdate): F[Unit] = {
    voiceState.guild_id.fold(
      Logger[F].debug(
        s"Voice state update without guild_id (from GUILD_CREATE): user=${voiceState.user_id}"
      )
    ) { guildId =>
      Logger[F].debug(
        s"[VOICE] State update: user=${voiceState.user_id}, channel=${voiceState.channel_id}"
      ) >>
        ...
        botUserIdRef.get.flatMap { botUserId =>
          Option
            .when(voiceState.user_id == botUserId)(
              voiceState.channel_id.fold(
                // Bot left voice channel - clean up pending connections
                ifEmpty = Logger[F].debug(
                  s"[VOICE] Bot left voice channel, cleaning up pending connections"
                ) >>
                ...
                _ =>
                  Logger[F].debug(
                    s"[VOICE] Bot state updated: session=${voiceState.session_id}, channel=${voiceState.channel_id}"
                  ) >>
                    pendingVoiceConnections.update { pending =>
                      val current =
                        pending.getOrElse(guildId, PendingVoiceConnection())
                      pending + (guildId -> current.copy(
                        sessionId = Some(voiceState.session_id),
            ...
            .getOrElse(
              Logger[F].debug(
                s"[VOICE] User ${voiceState.user_id} -> channel=${voiceState.channel_id}"
              )
            )
        }
    }
  ...
  def handleVoiceServerUpdate(voiceServer: VoiceServerUpdate): F[Unit] = {
    Logger[F].debug(
      s"[VOICE] Server update: guild=${voiceServer.guild_id}, endpoint=${voiceServer.endpoint}"
    ) >>
      // Update pending connection with token and endpoint
      pendingVoiceConnections.update { pending =>
        val current =
          pending.getOrElse(voiceServer.guild_id, PendingVoiceConnection())
        pending + (voiceServer.guild_id -> current.copy(
          token = Some(voiceServer.token),
  ...
  private def attemptVoiceConnection(guildId: String): F[Unit] = {
    pendingVoiceConnections.get.flatMap { pending =>
      pending
        ...
        .fold(
          Logger[F]
            .debug(s"[VOICE] No pending voice connection for guild $guildId")
        ) {
          case PendingVoiceConnection(
                Some(sessionId),
                Some(token),
                Some(endpoint),
                Some(channelId),
            ...
            Async[F]
              .delay(connectingVoiceConnectionGuilds.add(guildId))
              .flatMap {
                case false =>
                  Logger[F].debug(
                    s"[VOICE] Voice connection attempt already in progress for guild $guildId"
                  )
                case true =>
                  (pendingVoiceConnections.update { pending =>
                    pending
                      .get(guildId)
                      .fold(pending)(current =>
                        pending + (guildId -> current.copy(connecting = true))
                      )
                  } >>
                    Logger[F].info(
                      s"[VOICE] All voice connection data received for guild $guildId, connecting..."
                    ) >>
                    botUserIdRef.get.flatMap { botUserId =>
                      voiceGateway
                        ...
                        .flatMap { voiceWs =>
                          Logger[F].info(
                            s"[VOICE] Voice WebSocket obtained, storing in activeVoiceConnections for guild $guildId"
                          ) >>
                            activeVoiceConnections.update(
                              _ + (guildId -> voiceWs)
                            ) >>
                            activeVoiceConnections.get.flatMap { conns =>
                              Logger[F].info(
                                s"[VOICE] activeVoiceConnections now contains: ${conns.keys.mkString(", ")}"
                              )
                            } >>
                            pendingConnectionPromises.get.flatMap { promises =>
                              promises
                                ...
                                .fold(
                                  Logger[F].debug(
                                    s"[VOICE] No waiters for guild $guildId connection"
                                  )
                                ) { deferred =>
                                  Logger[F].info(
                                    s"[VOICE] Completing connection promise for guild $guildId (signaling waiters)"
                                  ) >>
                            ...
                            pendingVoiceConnections.update(_ - guildId) >>
                            Logger[F].info(
                              s"[VOICE] ✓ Successfully stored voice connection for guild $guildId"
                            )
                        }
                        .handleErrorWith { error =>
                          val msg = Option(error.getMessage).getOrElse(
                            error.getClass.getSimpleName
                          )
                          Logger[F].error(
                            s"[VOICE] Failed to connect to voice gateway: $msg"
                          ) >>
                            Logger[F].error(
                              s"[VOICE] Stack trace: ${error.getStackTrace.take(5).mkString("\n")}"
                            ) >>
                            pendingVoiceConnections.update(_ - guildId) >>
                            gatewayWebSocketRef.get.flatMap(
                              _.fold(Async[F].unit)(ws =>
                                leaveVoiceChannel(guildId, ws)
                              )
                            ) >>
                            pendingConnectionPromises.get.flatMap { promises =>
                              promises.get(guildId).traverse_ { deferred =>
                                val voiceError =
                                  VoiceConnectionFailed(guildId, msg)
                    ...
                    Async[F]
                      .delay(connectingVoiceConnectionGuilds.remove(guildId))
                      .void
                  )
              }
          case PendingVoiceConnection(
          ...
          case pendingConnection =>
            Logger[F].info(
              s"[VOICE] Waiting for more voice connection data for guild $guildId (sessionId: ${pendingConnection.sessionId.nonEmpty}, token: ${pendingConnection.token.nonEmpty}, endpoint: ${pendingConnection.endpoint.nonEmpty}, channelId: ${pendingConnection.channelId.nonEmpty}, connecting: ${pendingConnection.connecting})"
            )
        }
    }
  }
  ...
  def joinVoiceChannel(
    ...
    Logger[F].info(s"[VOICE] Joining channel...") >>
      ...
      gatewayWebSocketRef.set(
        Some(ws)
      ) >> // Store gateway WebSocket for later use (leaving channel)
  ...
  def waitForVoiceConnection(guildId: String): F[Unit] = {
    for {
      ...
      result <- deferred.get.timeoutTo(
        ...
        for {
          _ <- Logger[F].error(
            s"[VOICE] ✗ Timeout waiting for voice connection after 15 seconds"
          )
          connections <- activeVoiceConnections.get
          _ <- Logger[F].error(
            s"[VOICE] Current connections in map: ${connections.keys.mkString(", ")}"
          )
          _ <- Logger[F].error(s"[VOICE] Looking for guild: $guildId")
          _ <- Logger[F].error(
            s"[VOICE] This usually means the voice gateway connection failed or is taking too long"
          )
  ...
  def addToQueue(
      guildId: String,
      track: MusicTrack
  ): F[Either[QueueError, Unit]] = {
    musicQueueRef
      .modify { queues =>
        val currentQueue =
          queues.getOrElse(guildId, MusicQueue(List.empty, None, false))
      ...
      .flatTap {
        ...
        case Left(QueueFull(max)) =>
          Logger[F].warn(
            s"[QUEUE] Failed to add track: queue is full ($max tracks)"
          )
      }
  }
  ...
  def playNext(guildId: String): F[Option[MusicTrack]] = {
    musicQueueRef.modify { queues =>
      val currentQueue =
        queues.getOrElse(guildId, MusicQueue(List.empty, None, false))
      currentQueue.tracks.headOption
        .fold[(Map[String, MusicQueue], Option[MusicTrack])] {
          val updatedQueue =
            currentQueue.copy(currentTrack = None, isPlaying = false)
    ...
    for {
      ...
      _ <- musicQueueRef.update { queues =>
        val currentQueue =
          queues.getOrElse(guildId, MusicQueue(List.empty, None, false))
        val updatedQueue =
          currentQueue.copy(isPlaying = false, currentTrack = None)
      ...
      _ <- gatewayWebSocketRef.get.flatMap(
        _.fold(
          Logger[F].warn(
            s"[STOP] No gateway WebSocket available to send leave command"
          )
        ) { gatewayWs =>
      ...
      _ <- Logger[F].info(
        s"[STOP] ✓ Stopped music and disconnected from voice channel"
      )
    } yield ()
  }
  ...
  def abortActiveVoiceSessions(reason: String): F[Unit] = {
    for {
      ...
      _ <- fibers.toList.traverse_ { case (guildId, fiber) =>
        Logger[F].info(
          s"[VOICE] Canceling active playback for guild $guildId due to gateway reconnect"
        ) >>
          fiber.cancel
      }
      ...
      _ <- musicQueueRef.update(
        _.view.mapValues(_.copy(isPlaying = false, currentTrack = None)).toMap
      )
      _ <- voiceStateRef.set(None)
      _ <- voiceGateway
        .closeVoiceConnection(s"main gateway abort: $reason")
        .attempt
        .void
    } yield ()
  }
  ...
  def getQueue(guildId: String): F[MusicQueue] = {
    musicQueueRef.get.map(
      _.getOrElse(guildId, MusicQueue(List.empty, None, false))
    )
  }
    ...
    Logger[F].debug(s"[PLAYBACK] Starting playback") >>
      getQueue(guildId).flatMap { queue =>
        queue.currentTrack.fold(
          Logger[F].warn(
            s"[PLAYBACK] No current track to play - queue might be empty"
          )
        ) { track =>
          Logger[F].info(s"[PLAYBACK] Playing: ${track.title}") >>
            ...
            getStreamUrl(track.url).flatMap {
              case None =>
                Logger[F].error(
                  s"[PLAYBACK] Failed to extract stream URL (yt-dlp error or unavailable video)"
                ) >>
                ...
                Logger[F].debug(s"[PLAYBACK] Got stream URL") >>
                  activeVoiceConnections.get.flatMap { connections =>
                    connections
                      ...
                      .fold(
                        Logger[F].error(
                          s"[PLAYBACK] ✗ No active voice connection for guild $guildId - bot may not be connected to voice"
                        ) >>
                          clearCurrentTrack(guildId)
                      ) { voiceWs =>
  ...
  private def startPlaybackFiber(
      guildId: String,
      streamUrl: String,
      voiceWs: sttp.ws.WebSocket[F],
      startPosition: Int = 0
  ): F[Unit] = {
    val playbackAction = {
      val streamF = Option
        .when(startPosition > 0)(
          voiceGateway
            .streamAudioFromPosition(streamUrl, voiceWs, startPosition, guildId)
        )
        .getOrElse(
      ...
      streamF.handleErrorWith { error =>
        Logger[F].error(
          s"[PLAYBACK] ✗ Failed to stream audio: ${error.getMessage}"
        ) >>
          Logger[F].error(
            s"[PLAYBACK] Stack trace: ${error.getStackTrace.mkString("\n")}"
          ) >>
          clearCurrentTrack(guildId)
      } >>
          ...
          queue.tracks.headOption.fold(
            ...
            Logger[F].info(s"[PLAYBACK] Queue empty, leaving voice channel") >>
              gatewayWebSocketRef.get.flatMap(
                _.fold(
                  Logger[F].warn(
                    s"[PLAYBACK] No gateway WebSocket available to send leave command"
                  )
                ) { gatewayWs =>
                  leaveVoiceChannel(guildId, gatewayWs) >>
                    activeVoiceConnections.update(_ - guildId)
    ...
    Logger[F].info(s"[PLAYBACK] playNextTrack called for guild $guildId") >>
      playNext(guildId).flatMap(
        _.fold(
          ...
          Logger[F].info(
            s"[PLAYBACK] ✓ Got next track from queue: ${track.title}"
          ) >>
  ...
  def seek(guildId: String, positionSeconds: Int): F[Unit] = {
    getQueue(guildId).flatMap { queue =>
      queue.currentTrack.fold(
        ...
        Logger[F].info(
          s"[SEEK] Seeking to $positionSeconds seconds in ${track.title}"
        ) >>
          stopMusic(guildId) >>
          musicQueueRef.update { queues =>
            val currentQueue =
      ...
      _ <- queue.currentTrack match {
        ...
        case Some(track) =>
          for {
            ...
            _ <- Logger[F].info(
              s"[PAUSE] Pausing ${track.title} at ${currentPos}s"
            )
            _ <- stopMusic(guildId)
            _ <- musicQueueRef.update { queues =>
              val currentQueue =
  ...
  def resumePlayback(guildId: String): F[Unit] = {
    for {
      ...
      _ <- queue.currentTrack match {
        ...
        case Some(track) =>
          Logger[F].info(
            s"[RESUME] Resuming ${track.title} from ${queue.currentPosition}s"
          ) >>
            musicQueueRef.update { queues =>
              val currentQueue =
                queues.getOrElse(guildId, MusicQueue(List.empty, None, false))
  ...
  private def startPlayingCurrentWithPosition(
      guildId: String,
      startPosition: Int
  ): F[Unit] = {
    Logger[F].debug(
      s"[PLAYBACK] Starting playback from position $startPosition seconds"
    ) >>
      getQueue(guildId).flatMap { queue =>
        queue.currentTrack.fold(
          ...
          Logger[F].info(
            s"[PLAYBACK] Playing: ${track.title} from ${startPosition}s"
          ) >>
            getStreamUrl(track.url).flatMap {
              case None =>
              ...
              case Some(streamUrl) =>
                activeVoiceConnections.get.flatMap { connections =>
                  connections
                    ...
                    .fold(
                      Logger[F].error(
                        s"[PLAYBACK] ✗ No active voice connection for guild $guildId"
                      ) >>
                        clearCurrentTrack(guildId)
                    ) { voiceWs =>
                      startPlaybackFiber(
  ...
  def getCurrentPosition(guildId: String): F[Int] = {
    getQueue(guildId).map { queue =>
      queue.startTime.filter(_ => queue.isPlaying).fold(queue.currentPosition) {
        startTime =>
