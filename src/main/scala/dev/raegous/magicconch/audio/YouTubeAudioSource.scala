  ...
  override def extractTrackInfo(url: String): F[Option[AudioTrackInfo]] =
    Option.when(canHandle(url))(
      for {
        ...
        result = trackOpt.map(track => AudioTrackInfo(
          url = track.url,
          title = track.title,
        ...
        )(info => logger.info(s"[YouTubeAudioSource] Extracted: ${info.title}"))
      } yield result
  ...
  override def getStreamUrl(url: String): F[Option[String]] =
    Option.when(canHandle(url))(
      for {
        ...
        )(_ => logger.info(s"[YouTubeAudioSource] Successfully obtained stream URL"))
      } yield streamUrl
    ).getOrElse(Async[F].pure(none[String]))

  override def extractPlaylistUrls(url: String): F[List[String]] =
    Option.when(canHandle(url) && url.contains("playlist"))(
      for {
