...
trait AudioSource[F[_]] {
  ...
  /**
   * Extract individual track URLs from a playlist URL
   * Returns empty list if source doesn't support playlists or URL is not a playlist
   */
...
class AudioSourceManager[F[_]: Async](
  ...
  /**
   * Extract playlist URLs using the appropriate source
   * Returns empty list if no source can handle the URL or source doesn't support playlists
   */
  def extractPlaylistUrls(url: String): F[List[String]] = {
    findSource(url) match {
...
object AudioSourceManager {
  ...
  def make[F[_]: Async: fs2.io.process.Processes](
    using logger: org.typelevel.log4cats.Logger[F]
