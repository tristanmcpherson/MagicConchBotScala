...
object DaveSupport {
  ...
  final case class ExternalSender(signatureKey: Array[Byte], credential: Credential)
  ...
  final case class MlsKeyPackage(keyPackageMessage: Array[Byte]) extends DaveGatewayBinaryMessage {
  ...
  final class HkdfSenderKeyRatchet(baseSecret: Array[Byte]) extends SenderKeyRatchet {
    private var currentGeneration = 0
    private var currentSecret = baseSecret.clone()
    private val cachedKeys = scala.collection.mutable.Map.empty[Int, Array[Byte]]
    ...
    def keyFor(truncatedNonce: Long): Array[Byte] = synchronized {
      ...
      while (currentGeneration <= generation) {
        ...
        cachedKeys.update(
          currentGeneration,
          MlsPrimitives.expandWithLabel(secretForGeneration, "key", generationContext, AesKeyBytes)
        )
        currentSecret = MlsPrimitives.expandWithLabel(secretForGeneration, "secret", generationContext, 32)
        currentGeneration += 1
      }

      cachedKeys.getOrElse(
        generation,
        throw new IllegalArgumentException(s"DAVE sender key generation $generation has been erased or was not derived")
      ).clone()
    }
  }
  ...
  trait DaveMlsBackend {
    ...
    def processProposals(payload: Array[Byte], recognizedUserIds: Set[String]): Option[MlsCommitWelcome]
    def processCommit(commitMessage: Array[Byte]): Boolean
    def processWelcome(welcomeMessage: Array[Byte], recognizedUserIds: Set[String]): Boolean
  ...
  final class UnsupportedMlsBackend extends DaveMlsBackend {
    private def fail[A]: A =
      throw new UnsupportedOperationException("DAVE MLS support requires an RFC 9420 MLS backend")

    def reset(): Unit = ()
    def keyPackageMessage(): Array[Byte] = fail
    def setExternalSender(externalSender: ExternalSender): Unit = fail
    def processProposals(payload: Array[Byte], recognizedUserIds: Set[String]): Option[MlsCommitWelcome] = fail
    def processCommit(commitMessage: Array[Byte]): Boolean = fail
    def processWelcome(welcomeMessage: Array[Byte], recognizedUserIds: Set[String]): Boolean = fail
    ...
    def encode(message: DaveGatewayBinaryMessage): Array[Byte] =
      message match {
        ...
        case MlsCommitWelcome(commitMessage, welcomeMessage) =>
          Array(OpMlsCommitWelcome.toByte) ++ commitMessage ++ welcomeMessage.getOrElse(Array.emptyByteArray)

        case MlsAnnounceCommitTransition(sequenceNumber, transitionId, commitMessage) =>
          val out = new ByteArrayOutputStream()
          writeUInt16(out, sequenceNumber)
      ...
      opcode match {
        case OpMlsExternalSenderPackage =>
          for {
            ...
            _ <- Either.cond(payload.length >= credentialOffset + 2, (), "External sender missing credential type")
        ...
        case OpMlsProposals =>
          for {
            _ <- Either.cond(payload.nonEmpty, (), "DAVE proposals message missing operation type")
        ...
        case OpMlsAnnounceCommitTransition =>
          if (payload.length < 2) Left("DAVE announce commit transition missing transition ID")
          else Right(MlsAnnounceCommitTransition(sequenceNumber, readUInt16(payload, 0), payload.drop(2)))

        case OpMlsWelcome =>
          if (payload.length < 2) Left("DAVE welcome missing transition ID")
          else Right(MlsWelcome(sequenceNumber, readUInt16(payload, 0), payload.drop(2)))
  ...
  object MediaFrameCodec {
    ...
    def parse(frame: Array[Byte]): Either[String, ParsedProtocolFrame] = {
      if (frame.length < minSupplementalSize) Left("Frame is too short for DAVE supplemental data")
      else if (frame(frame.length - 2) != MagicHi || frame(frame.length - 1) != MagicLo) Left("DAVE magic marker missing")
      else {
        val supplementalSize = frame(frame.length - 3) & 0xFF
        val supplementalStart = frame.length - supplementalSize
        if (supplementalSize < minSupplementalSize) Left(s"DAVE supplemental size too small: $supplementalSize")
        else if (supplementalStart <= 0 || supplementalStart >= frame.length - 3) Left(s"Invalid DAVE supplemental size: $supplementalSize")
        else {
          val supplementalPayloadEnd = frame.length - 3
          val interleavedFrame = frame.take(supplementalStart)
          val tag = frame.slice(supplementalStart, supplementalStart + TruncatedTagBytes)
          readUleb128(frame, supplementalStart + TruncatedTagBytes, supplementalPayloadEnd).flatMap {
            case (nonce, rangesOffset) =>
              readRanges(frame, rangesOffset, supplementalPayloadEnd, interleavedFrame.length).map { ranges =>
                ParsedProtocolFrame(
                  interleavedFrame,
    ...
    def encryptAudioFrame(frame: Array[Byte], truncatedNonce: Long, ratchet: SenderKeyRatchet): Array[Byte] =
      encryptFrame(frame, truncatedNonce, Nil, ratchet)

    def diagnostics(inputLength: Int, encryptedFrame: Array[Byte]): Either[String, EncryptedAudioFrameDiagnostics] =
      parse(encryptedFrame).map { parsed =>
        EncryptedAudioFrameDiagnostics(
          ...
          markerPresent = encryptedFrame.takeRight(2).sameElements(Array(MagicHi, MagicLo)),
    ...
    def encryptFrame(
      ...
      val cipherOutput = aesGcmEncrypt(ratchet.keyFor(truncatedNonce), truncatedNonce, encryptedRangePlaintext, additionalData)
      ...
      val supplementalSize = tag.length + nonceBytes.length + rangeBytes.length + 1 + 2

      require(supplementalSize <= 255, s"DAVE supplemental data too large: $supplementalSize")

      interleaved ++ tag ++ nonceBytes ++ rangeBytes ++ Array(supplementalSize.toByte, MagicHi, MagicLo)
    }

    def decryptFrame(frame: Array[Byte], ratchet: SenderKeyRatchet): Either[String, Array[Byte]] =
      parse(frame).flatMap { parsed =>
        val aad = associatedData(parsed.interleavedFrame, parsed.footer.unencryptedRanges)
        val encryptedPart = encryptedBytes(parsed.interleavedFrame, parsed.footer.unencryptedRanges)
        val tagPadded = parsed.footer.authTag ++ Array.fill[Byte](TruncatedTagBytes)(0)
        try {
          ...
          Right(interleave(parsed.interleavedFrame, plaintext, parsed.footer.unencryptedRanges))
        } catch {
          case e: Exception => Left(s"DAVE frame decrypt failed: ${e.getMessage}")
        }
      }

    private def minSupplementalSize: Int = TruncatedTagBytes + 1 + 1 + 2

    private def aesGcmEncrypt(key: Array[Byte], truncatedNonce: Long, plaintext: Array[Byte], aad: Array[Byte]): Array[Byte] = {
      require(key.length == AesKeyBytes, s"DAVE AES-GCM media key must be $AesKeyBytes bytes")
      val cipher = Cipher.getInstance("AES/GCM/NoPadding")
      cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), new GCMParameterSpec(GcmTagBits, fullNonce(truncatedNonce)))
      cipher.updateAAD(aad)
      cipher.doFinal(plaintext)
    }

    private def aesGcmDecrypt(key: Array[Byte], truncatedNonce: Long, ciphertextAndTag: Array[Byte], aad: Array[Byte]): Array[Byte] = {
      require(key.length == AesKeyBytes, s"DAVE AES-GCM media key must be $AesKeyBytes bytes")
      val cipher = GCMBlockCipher.newInstance(AESEngine.newInstance())
      cipher.init(false, new AEADParameters(new KeyParameter(key), TruncatedGcmTagBits, fullNonce(truncatedNonce), aad))
      val output = new Array[Byte](cipher.getOutputSize(ciphertextAndTag.length))
      val processed = cipher.processBytes(ciphertextAndTag, 0, ciphertextAndTag.length, output, 0)
  ...
  def fullNonce(truncatedNonce: Long): Array[Byte] = {
    require(truncatedNonce >= 0 && truncatedNonce <= 0xFFFFFFFFL, s"DAVE truncated nonce out of uint32 range: $truncatedNonce")
  ...
  def readUleb128(bytes: Array[Byte], offset: Int, limit: Int): Either[String, (Long, Int)] = {
    var result = 0L
    var shift = 0
    var i = offset
  ...
  private def readRanges(
    ...
    while (i < limit) {
      val parsed = for {
        ...
        i = nextLength
        ranges = ranges :+ UnencryptedRange(rangeOffset.toInt, rangeLength.toInt)
      }
  ...
  private def validateRanges(ranges: List[UnencryptedRange], frameLength: Int): Either[String, Unit] = {
    var previousEnd = 0
    ranges.foldLeft[Either[String, Unit]](Right(())) {
      case (Left(error), _) => Left(error)
      case (Right(_), range) =>
        if (range.offset < previousEnd) Left("DAVE unencrypted ranges overlap or are unordered")
        else if (range.endExclusive > frameLength) Left("DAVE unencrypted range exceeds frame length")
        else {
  ...
  private def encryptedBytes(frame: Array[Byte], unencryptedRanges: List[UnencryptedRange]): Array[Byte] = {
    val out = new ByteArrayOutputStream()
    var cursor = 0
  ...
  private def associatedData(frame: Array[Byte], unencryptedRanges: List[UnencryptedRange]): Array[Byte] = {
    val out = new ByteArrayOutputStream()
    unencryptedRanges.foreach(range => out.write(frame, range.offset, range.length))
    out.toByteArray
  }

  private def interleave(template: Array[Byte], encryptedOrPlaintext: Array[Byte], unencryptedRanges: List[UnencryptedRange]): Array[Byte] = {
    ...
    unencryptedRanges.foreach { range =>
      ...
      if (encryptedLength > 0) {
        System.arraycopy(encryptedOrPlaintext, sourceCursor, out, cursor, encryptedLength)
        sourceCursor += encryptedLength
      }
      cursor = range.endExclusive
    }
    if (cursor < out.length) {
      System.arraycopy(encryptedOrPlaintext, sourceCursor, out, cursor, out.length - cursor)
    }
    out
  }

  private def writeOpaqueVar(out: ByteArrayOutputStream, bytes: Array[Byte]): Unit = {
    out.write(writeMlsVarint(bytes.length))
    out.write(bytes)
  }

  private def readOpaqueVar(bytes: Array[Byte], offset: Int): Either[String, (Array[Byte], Int)] =
  ...
  private def readMlsVarint(bytes: Array[Byte], offset: Int): Either[String, (Int, Int)] = {
    if (offset >= bytes.length) Left("Missing MLS varint length")
      ...
      (first >> 6) match {
        ...
        case 1 =>
          ...
          else Right((((first & 0x3F) << 8) | (bytes(offset + 1) & 0xFF), offset + 2))
        case 2 =>
  ...
  private def readUInt16(bytes: Array[Byte], offset: Int): Int =
    ByteBuffer.wrap(bytes, offset, 2).order(ByteOrder.BIG_ENDIAN).getShort() & 0xFFFF
  ...
  private def expandWithLabel(secret: Array[Byte], label: String, context: Array[Byte], length: Int): Array[Byte] = {
    val fullLabel = s"MLS 1.0 $label".getBytes("UTF-8")
    val info = uint16Bytes(length) ++ opaque8(fullLabel) ++ opaqueVarForHkdf(context)
    hkdfExpand(secret, info, length)
  }

  private def hkdfExpand(secret: Array[Byte], info: Array[Byte], length: Int): Array[Byte] = {
