...
object MlsMessages {
    ...
    def userId: Option[String] =
      Option.when(credentialType == 1 && credentialIdentity.length == 8) {
        java.lang.Long.toUnsignedString(
          java.nio.ByteBuffer.wrap(credentialIdentity).getLong
        )
      }
  }
  ...
  final case class RawProposal(proposalType: Int, payload: Array[Byte])
      extends Proposal
  ...
  final case class Welcome(
      cipherSuite: Int,
      secrets: List[WelcomeSecret],
      encryptedGroupInfo: Array[Byte]
  )
  final case class WelcomeSecret(
      newMember: Array[Byte],
      kemOutput: Array[Byte],
  ...
  def parsePublicMessage(
      message: MlsMessage
  ): Either[DecodeError, PublicMessage] =
    if (message.wireFormat != WireFormatPublicMessage)
      Left(
        DecodeError(
          s"expected public message, got wire format ${message.wireFormat}"
        )
      )
    else
      {
        ...
        for {
          ...
          confirmation <-
            if (content.contentType == ContentTypeCommit)
              reader.readOpaqueVar().map(Some(_))
            else Right(None)
          ...
          _ <-
            if (reader.atEnd) Right(())
            else
              Left(
                DecodeError(
                  s"trailing public-message bytes: ${reader.remaining}"
                )
              )
        } yield PublicMessage(
          content,
          ...
          confirmation,
          membership,
          uint16(message.version) ++ uint16(message.wireFormat) ++ reader
            .slice(contentStart, contentEnd)
        )
      }.orElse(parseDaveCompactExternalProposal(message))

  private def readRawPublicMessage(
      reader: Reader
  ): Either[DecodeError, (PublicMessage, Array[Byte])] = {
    ...
    for {
      ...
      _ <- Either.cond(
        ...
        (),
        DecodeError(s"expected public message, got wire format $wireFormat")
      )
      ...
      val tbs = uint16(version) ++ uint16(wireFormat) ++ reader.slice(
        contentStart,
        contentEnd
      )
  ...
  def parseProposalBatch(
      bytes: Array[Byte]
  ): Either[DecodeError, ProposalBatch] = {
    val reader = Reader(bytes)
    for {
      op <- reader.readUInt8()
      batch <- op match {
        case 0 =>
          parseAppendProposalMessages(
            reader
              .readBytes(reader.remaining)
              .toOption
              .getOrElse(Array.emptyByteArray)
          )
        case 1 =>
          parseRevokedProposalRefs(
            reader
              .readBytes(reader.remaining)
              .toOption
              .getOrElse(Array.emptyByteArray)
  ...
  private def parseAppendProposalMessages(
      bytes: Array[Byte]
  ): Either[DecodeError, ProposalBatch] = {
      ...
      vectorReader
        ...
        .flatMap(messages =>
          if (vectorReader.atEnd) Right(messages)
            ...
            Left(
              DecodeError(
                s"trailing vector proposal bytes: ${vectorReader.remaining}"
              )
            )
        )
        .map(messages =>
    ...
    vectorResult
      ...
      .orElse {
        parseMlsMessage(bytes)
          ...
          .map(message =>
            ProposalBatch(isRevoke = false, List(message), Nil, List(bytes))
          )
      }
  }

  private def parseOpaqueProposalMessageVector(
      bytes: Array[Byte]
  ): Either[DecodeError, ProposalBatch] = {
    ...
    for {
      ...
      _ <-
        if (outerReader.atEnd) Right(())
        else
          Left(
            DecodeError(
              s"trailing outer opaque proposal bytes: ${outerReader.remaining}"
            )
          )
  ...
  private def readOpaqueProposalMessages(
      bytes: Array[Byte]
  ): Either[DecodeError, List[(PublicMessage, Array[Byte])]] = {
    val reader = Reader(bytes)
    val out =
      scala.collection.mutable.ListBuffer.empty[(PublicMessage, Array[Byte])]
    while (!reader.atEnd) {
      val parsed = for {
  ...
  private def parseOpaqueSingleProposalMessage(
      bytes: Array[Byte]
  ): Either[DecodeError, ProposalBatch] = {
    val reader = Reader(bytes)
    for {
      ...
      _ <-
        if (reader.atEnd) Right(())
          ...
          Left(
            DecodeError(s"trailing opaque proposal bytes: ${reader.remaining}")
          )
      message <- parseMlsMessage(messageBytes).flatMap(parsePublicMessage)
    } yield ProposalBatch(
      isRevoke = false,
  ...
  private def parseDaveCompactExternalProposal(
      message: MlsMessage
  ): Either[DecodeError, PublicMessage] =
    if (message.wireFormat != WireFormatPublicMessage)
      Left(
      ...
      for {
        ...
        _ <- Either.cond(
          sender.senderType == SenderTypeExternal,
          (),
          DecodeError("compact DAVE proposal sender is not external")
        )
        ...
        content <- contentType match {
          ...
          case other               =>
            Left(
              DecodeError(
                s"unsupported compact DAVE framed content type: $other"
              )
            )
        }
        contentEnd = reader.position
        ...
        _ <-
          if (reader.atEnd) Right(())
            ...
            Left(
              DecodeError(
                s"trailing compact DAVE proposal bytes: ${reader.remaining}"
              )
            )
      } yield {
        val contentWithEmptyAuthData =
          reader.slice(
            contentStart,
            contentStart + groupId.length + MlsPrimitives
              .varint(groupId.length)
              .length + 8 + 5
          ) ++
        ...
        PublicMessage(
          ...
          membershipTag = None,
          tbs = uint16(message.version) ++ uint16(
            message.wireFormat
          ) ++ contentWithEmptyAuthData
        )
      }
    }

  private def parseRevokedProposalRefs(
      bytes: Array[Byte]
  ): Either[DecodeError, ProposalBatch] = {
    val vectorReader = Reader(bytes)
    val vectorResult =
      vectorReader
        ...
        .flatMap(refs =>
          if (vectorReader.atEnd) Right(refs)
            ...
            Left(
              DecodeError(
                s"trailing vector revoke bytes: ${vectorReader.remaining}"
              )
            )
        )
      ...
      if (bytes.length > 0 && bytes.length % HashLength == 0) {
        Right(
          ProposalBatch(
            isRevoke = true,
            Nil,
            bytes.grouped(HashLength).map(_.toArray).toList
          )
        )
      } else {
        Left(
          DecodeError(
            s"invalid DAVE revoke proposal-ref payload length: ${bytes.length}"
          )
        )
      }
    }
  ...
  def parseProposal(content: Array[Byte]): Either[DecodeError, Proposal] = {
    ...
    for {
      ...
      proposal <- proposalType match {
        ...
        case other              =>
          reader.readBytes(reader.remaining).map(RawProposal(other, _))
      }
    } yield proposal
  }

  def parseKeyPackageMessage(
      bytes: Array[Byte]
  ): Either[DecodeError, KeyPackage] =
    parseWrappedKeyPackageMessage(bytes).orElse(parseKeyPackagePayload(bytes))

  def parseKeyPackagePayload(
      bytes: Array[Byte]
  ): Either[DecodeError, KeyPackage] = {
    val reader = Reader(bytes)
    parseKeyPackage(reader).flatMap(keyPackage =>
      Either.cond(
        reader.atEnd,
        keyPackage,
        DecodeError(s"trailing key-package bytes: ${reader.remaining}")
      )
    )
  }
  ...
  def parseWelcomeMessage(bytes: Array[Byte]): Either[DecodeError, Welcome] =
    parseMlsMessage(bytes).flatMap { message =>
      if (message.wireFormat != WireFormatWelcome)
        Left(
          DecodeError(s"expected welcome message, got ${message.wireFormat}")
        )
  ...
  def verifyPublicMessageSignature(
  ...
  def parseCommit(content: Array[Byte]): Either[DecodeError, Commit] = {
    ...
    for {
      ...
      _ <- hasPathByte match {
        ...
        case other =>
          Left(DecodeError(s"invalid MLS optional UpdatePath marker: $other"))
      }
      _ <-
        if (reader.atEnd) Right(())
        else Left(DecodeError(s"trailing commit bytes: ${reader.remaining}"))
    } yield Commit(proposals, hasPathByte == 1)
  }

  private def parseFramedContent(
      reader: Reader
  ): Either[DecodeError, FramedContent] =
    for {
      groupId <- reader.readOpaqueVar()
      epoch <- reader.readUInt64()
      sender <- parseSender(reader)
      ...
      content <- contentType match {
        ...
        case other                  =>
          Left(DecodeError(s"unsupported framed content type: $other"))
      }
    } yield FramedContent(
      groupId,
      epoch,
  ...
  private def parseSender(reader: Reader): Either[DecodeError, Sender] =
    reader.readUInt8().flatMap {
      case 0 =>
        reader.readUInt32().map(idx => Sender(SenderTypeExternal, Some(idx)))
      case SenderTypeMember =>
        reader.readUInt32().map(idx => Sender(SenderTypeMember, Some(idx)))
      case SenderTypeExternal =>
        reader.readUInt32().map(idx => Sender(SenderTypeExternal, Some(idx)))
      case SenderTypeNewMemberProposal =>
        Right(Sender(SenderTypeNewMemberProposal, None))
      case SenderTypeNewMemberCommit =>
        Right(Sender(SenderTypeNewMemberCommit, None))
      case other => Left(DecodeError(s"unsupported sender type: $other"))
    }

  private def parseKeyPackage(
      reader: Reader
  ): Either[DecodeError, KeyPackage] = {
    val start = reader.position
    for {
  ...
  private def parseWrappedKeyPackageMessage(
      bytes: Array[Byte]
  ): Either[DecodeError, KeyPackage] =
    parseMlsMessage(bytes).flatMap(message =>
      Either
        .cond(
          message.wireFormat == WireFormatKeyPackage,
          message.body,
          DecodeError(
            s"expected key package message, got ${message.wireFormat}"
          )
        )
  ...
  private def parseCapabilities(
      reader: Reader
  ): Either[DecodeError, Capabilities] =
    for {
  ...
  private def parseWelcome(reader: Reader): Either[DecodeError, Welcome] =
    for {
      ...
      _ <-
        if (reader.atEnd) Right(())
        else Left(DecodeError(s"trailing welcome bytes: ${reader.remaining}"))
    } yield Welcome(cipherSuite, secrets, encryptedGroupInfo)

  private def parseWelcomeSecret(
      reader: Reader
  ): Either[DecodeError, WelcomeSecret] =
    for {
  ...
  private def parseProposalOrRef(
      reader: Reader
  ): Either[DecodeError, ProposalOrRef] =
    reader.readUInt8().flatMap {
      case ProposalOrRefTypeProposal =>
        parseProposalFromReader(reader).map(InlineProposal.apply)
      case ProposalOrRefTypeReference =>
        reader.readBytes(HashLength).map(ProposalReference.apply)
      case other => Left(DecodeError(s"unsupported ProposalOrRef type: $other"))
    }

  private def parseProposalFromReader(
      reader: Reader
  ): Either[DecodeError, Proposal] =
    for {
      proposalType <- reader.readUInt16()
      proposal <- proposalType match {
        case ProposalTypeAdd => parseKeyPackage(reader).map(AddProposal.apply)
  ...
  private def readProposalBytes(
      reader: Reader
  ): Either[DecodeError, Array[Byte]] = {
    val start = reader.position
    parseProposalFromReader(reader).map(_ =>
      reader.slice(start, reader.position)
    )
  }

  private def readCommitBytes(
      reader: Reader
  ): Either[DecodeError, Array[Byte]] = {
    val start = reader.position
    for {
  ...
  private def verifyWithLabel(
    ...
    try {
      ...
      val signedContent =
        MlsPrimitives.opaqueVar(fullLabel) ++ MlsPrimitives.opaqueVar(content)
      val verifier =
  ...
  private def ecPublicKey(uncompressed: Array[Byte]): ECPublicKey = {
    require(
      uncompressed.length == 65 && uncompressed(0) == 0x04.toByte,
    ...
    val params = paramsGenerator.getParameterSpec(
      classOf[java.security.spec.ECParameterSpec]
    )
