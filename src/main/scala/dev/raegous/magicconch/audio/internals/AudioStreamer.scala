...
object AudioStreamer {
  def make[F[_]: Async: Processes](
    ...
    for {
      ...
      pcmPool <- BufferPool
        .shorts[F](poolSize = 10, bufferSize = 1920) // 960 samples × 2 channels
      opusPool <- BufferPool
        .bytes[F](poolSize = 10, bufferSize = 1000) // 1KB per frame
      ...
      audioStreamer <- Resource.eval {
        for {
          ...
          encoder <- Async[F].delay {
            val enc =
              new OpusEncoder(48000, 2, OpusApplication.OPUS_APPLICATION_AUDIO)
...
class AudioStreamer[F[_]: Async: Processes] private (
  ...
  private val PCM_FRAME_SIZE_BYTES =
    OPUS_FRAME_SIZE * CHANNELS * BYTES_PER_SAMPLE // 3840 bytes
    ...
    Logger[F].debug(s"[AUDIO] Starting audio stream") >>
      ...
      createAudioPipeline(streamUrl, startPosition = None)
        ...
        .handleErrorWith { error =>
          Logger[F].error(
            s"[AUDIO] ✗ Audio streaming failed: ${error.getMessage}"
          ) >>
            Logger[F].error(
              s"[AUDIO] Stack trace: ${error.getStackTrace.take(10).mkString("\n")}"
            )
        }
  }
  ...
  /** Creates FFmpeg pipeline to convert audio to PCM frames.
    ...
    *   URL to stream from
    * @param startPosition
    *   Optional start position in seconds for seeking
    */
  private def createAudioPipeline(
      streamUrl: String,
  ...
  private def createFfmpegAudioPipeline(
      streamUrl: String,
      startPosition: Option[Int]
  ): Stream[F, Array[Byte]] = {
    ...
    val seekCommand =
      startPosition.map(pos => List("-ss", pos.toString)).getOrElse(Nil)
    val readRateCommand = List(
      "-readrate",
      FfmpegReadRate.toString
    )
    ...
    val ffmpegCommand =
      baseCommand ++ seekCommand ++ readRateCommand ++ remainingCommand
    ...
    Stream.eval(Logger[F].debug(s"[AUDIO] Starting FFmpeg$positionMsg")) >>
      Stream
        .resource(
          ProcessBuilder(ffmpegCommand.head, ffmpegCommand.tail*).spawn[F]
        )
        .flatMap { process =>
  ...
  /** Buffers PCM data to ensure only complete frames are emitted. Accumulates
    * bytes until we have exactly frameSize bytes, then emits them. Any
    * remaining partial frame at the end is discarded.
    */
  private def bufferCompleteFrames(
      frameSize: Int
  ): fs2.Pipe[F, Byte, fs2.Chunk[Byte]] = { stream =>
  ...
  /** Process PCM frames through the paced Opus queue, ordered packetizer, and
    * isolated UDP sender.
    */
  private def processAudioFrames(
    ...
    Stream.eval {
      for {
        ...
        pacedFrameQueue <- Queue.bounded[F, Option[PacedFrame]](
          PacedQueueFrames
        )
        packetQueue <- Queue.bounded[F, Option[PacketizedFrame]](
          PacketQueueFrames
        )
        ...
        rtpTransportStateRef <- Ref.of[F, RtpTransportState](
          RtpTransportState.initial
        )
        _ <- {
          val program = for {
            producer <- produceAudioFrames(
              input,
              frameQueue,
              depthRef,
              sourceDoneRef,
          ...
          program.handleErrorWith { error =>
            Logger[F].error(
              s"[AUDIO] Audio frame processing failed: ${error.getMessage}"
            ) >>
      ...
      .evalMap { frame =>
        encodeFrameForQueue(frame, metricsRef, guildId).flatMap { opusFrame =>
          frameQueue.offer(Some(opusFrame)) >>
            depthRef
              .updateAndGet(depth => (depth + 1).min(MaxQueueFrames))
              .flatMap { depth =>
                metricsRef.update(metrics =>
                  metrics.copy(
  ...
  private def encodeFrameForQueue(
    ...
    val invalidPcmLog = Option
      .when(pcmData.length != PCM_FRAME_SIZE_BYTES)(
        Logger[F].warn(
          s"[AUDIO] Invalid PCM frame size: ${pcmData.length} bytes (expected $PCM_FRAME_SIZE_BYTES); sending Opus silence"
        )
      )
    ...
    Option
      .when(pcmData.length == PCM_FRAME_SIZE_BYTES) {
        for {
          ...
          _ <- metricsRef.update(metrics =>
            metrics.copy(maxEncodeMs = metrics.maxEncodeMs.max(encodeMs))
          )
  ...
  private def awaitInitialBuffer(
      depthRef: Ref[F, Int],
      sourceDoneRef: Ref[F, Boolean]
  ): F[Unit] =
    (depthRef.get, sourceDoneRef.get).flatMapN { (depth, sourceDone) =>
      Either
        .cond(depth >= TargetQueueFrames || sourceDone, (), ())
        .fold(
          _ =>
            Async[F]
              .sleep(10.millis) >> awaitInitialBuffer(depthRef, sourceDoneRef),
          _ => Async[F].unit
        )
    }
  ...
  private def pacedSenderTick(
    ...
    Clock[F].monotonic.flatMap { now =>
      ...
      Option
        .when(wait > Duration.Zero)(Async[F].sleep(wait))
        .getOrElse(Async[F].unit) >>
        paceNextFrame(
          frameQueue,
          depthRef,
  ...
  private def paceNextFrame(
    ...
    frameQueue.tryTake.flatMap {
      ...
      case Some(None) =>
        Logger[F].debug(
          "[AUDIO] Audio source completed and jitter buffer drained"
        ) >>
          enqueueTerminal(
            pacedFrameQueue,
            pacedDepthRef,
            ...
            metricsRef,
            metrics =>
              metrics.copy(droppedPacedFrames =
                metrics.droppedPacedFrames + 1L
              ),
            (metrics, depth) =>
              metrics.copy(pacedQueueHighWaterDepth =
                metrics.pacedQueueHighWaterDepth.max(depth)
              )
          ) >>
          Async[F].pure(true)
          ...
          Option
            ...
            .getOrElse(
              enqueueTerminal(
                ...
                metrics =>
                  metrics
                    .copy(droppedPacedFrames = metrics.droppedPacedFrames + 1L),
                (metrics, depth) =>
                  metrics.copy(pacedQueueHighWaterDepth =
                    metrics.pacedQueueHighWaterDepth.max(depth)
                  )
  ...
  private def enqueuePacedFrame(
    ...
    for {
      ...
      transportState <- rtpTransportStateRef.modify(state =>
        (state.next(OPUS_FRAME_SIZE), state)
      )
      ...
      _ <- offerWithDropOldest(
        ...
        metrics =>
          metrics.copy(droppedPacedFrames = metrics.droppedPacedFrames + 1L)
      )
    } yield ()
      ...
      case None =>
        pacedDepthRef.update(depth => (depth - 1).max(0)) >>
          enqueueTerminal(
            ...
            metricsRef,
            metrics =>
              metrics.copy(droppedPacketFrames =
                metrics.droppedPacketFrames + 1L
              ),
            (metrics, depth) =>
              metrics.copy(packetQueueHighWaterDepth =
                metrics.packetQueueHighWaterDepth.max(depth)
              )
          )
    }
  ...
  private def packetizeFrame(
    ...
    for {
      ...
      mediaPayload <- daveAudioEncryptor.fold(
        Async[F].pure(pacedFrame.opusData)
      )(_(pacedFrame.opusData))
      ...
      _ <- offerWithDropOldest(
        ...
        metrics =>
          metrics.copy(droppedPacketFrames = metrics.droppedPacketFrames + 1L)
      )
    } yield ()
  }
  ...
  private def sendPacketizedFrame(
    ...
    sendUDPPacket(
      ...
      recordFrameMetrics(
        ...
        pipelineMs = nanosToMillis(
          udpTiming.completedAtNanos - packetizedFrame.pacedAtNanos
        ),
        encodeMs = 0L,
  ...
  private def offerWithDropOldest[A](
    ...
    queue.tryOffer(item).flatMap {
      case true =>
        depthRef.updateAndGet(depth => (depth + 1).min(capacity)).flatMap {
          depth =>
      ...
      case false =>
        queue.tryTake.flatMap {
          ...
          case None =>
            Async[F].cede >> offerWithDropOldest(
              queue,
              item,
              depthRef,
              capacity,
    ...
    lastSendNanosRef
      .modify(last =>
        (
          Some(sendNanos),
          last
            .map(previous => nanosToMillis(sendNanos - previous))
            .getOrElse(OPUS_FRAME_DURATION.toLong)
        )
      )
      .flatMap { interSendMs =>
        metricsRef
          .modify { metrics =>
            val next = metrics.copy(
              ...
              underflowFrames = metrics.underflowFrames + Option
                .when(underflow)(1L)
                .getOrElse(0L),
              emptyOpusFrames = metrics.emptyOpusFrames + Option
                .when(emptyOpus)(1L)
                .getOrElse(0L),
            ...
            val shouldLog =
              next.sentFrames % MetricsLogFrameInterval == 0L || underflow || interSendMs > 30L || pipelineMs > 18L || udpMs > 10L
          ...
          opusPool.use { opusOutput =>
            Async[F].delay {
              ...
              while (i < pcmShorts.length) {
                ...
                pcmShorts(i) = Math
                  .max(Short.MinValue, Math.min(Short.MaxValue, amplified))
                  .toShort
                i += 1
              }
              ...
              Option
                .when(encodedLength > 0)(opusOutput.take(encodedLength))
  ...
  private def encryptPacket(
    ...
    Async[F]
      ...
      .handleErrorWith { error =>
        Logger[F].error(
          s"[AUDIO] ✗ Failed to encrypt packet: ${error.getMessage}"
        ) >>
          Logger[F].error(
            s"[AUDIO] Stack trace: ${error.getStackTrace.take(5).mkString("\n")}"
          ) >>
          Async[F].raiseError(error)
      }
  }

  private final case class UdpSendTiming(
      completedAtNanos: Long,
      innerDurationNanos: Long
  )

  private def sendUDPPacket(
    ...
    Async[F]
      .blocking {
        ...
        UdpSendTiming(
          completedAtNanos = sendEnd,
          innerDurationNanos = sendEnd - sendStart
        )
      }
      .handleErrorWith { error =>
        Logger[F].error(
          s"[AUDIO] Failed to send UDP packet to $ip:$port: ${error.getMessage}"
        ) >>
          Async[F].raiseError(error)
      }
  }
  ...
  def streamAudioFromPosition(
    ...
    Logger[F].debug(
      s"[AUDIO] Starting stream from position: ${startPosition}s"
    ) >>
