...
object VoiceProtocol {
  ...
  /** RTP Header for Discord voice packets. Always 12 bytes, used as AAD
    * (Additional Authenticated Data) in AEAD encryption.
    *
  ...
  object RTPHeader {
    ...
    /** Encode RTP header to bytes. Returns Array[Byte] for easy interop with
      * encryption APIs.
      */
  ...
  object Nonce {
    ...
    /** Create a 24-byte nonce for XChaCha20-Poly1305 (rtpsize mode). Format:
      * 4-byte counter (big-endian) + 20 zero bytes
      */
    ...
    /** Create a 12-byte nonce for AES256-GCM (rtpsize mode). Format: 4-byte
      * counter (big-endian) + 8 zero bytes
      */
  ...
  /** Complete RTP packet ready for UDP transmission. Format: [Header (12B)] +
    * [Encrypted Payload] + [Nonce Counter (4B)]
    */
  case class RTPPacket(
      header: RTPHeader,
      encryptedPayload: Array[
        Byte
      ], // Opus data encrypted with 16-byte auth tag
    ...
    def toBytes: Array[Byte] = {
      ...
      val result =
        new Array[Byte](headerBytes.length + encryptedPayload.length + 4)
      ...
      System.arraycopy(
        ...
        0,
        result,
        headerBytes.length + encryptedPayload.length,
        4
      )
      result
    }
