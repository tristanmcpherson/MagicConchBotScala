  ...
  def make[F[_]: Logger: Async: fs2.io.process.Processes](
    ...
    val allocate: F[VoiceGateway[F]] =
      for {
        ...
        shutdownSignalRef <- Ref.of[F, Option[Deferred[F, Unit]]](
          Some(shutdownSignal)
        )
        ...
        daveOutboundDiagnosticsRef <- Ref.of[F, DaveOutboundDiagnostics](
          DaveOutboundDiagnostics.initial
        )
    ...
    Resource.make(allocate)(
      _.closeVoiceConnection("resource finalizer").attempt.void
    )
  }
}
...
class VoiceGateway[F[_]: Async: fs2.io.process.Processes] private (
    ...
    private val daveOutboundDiagnosticsRef: Ref[
      F,
      VoiceGateway.DaveOutboundDiagnostics
    ]
)(using Logger[F]) {
  ...
  def connectToVoiceGateway(
    ...
    for {
      _ <- closeVoiceConnection(
        "replacing existing voice connection before connect"
      )
      ...
      _ <- Async[F].start {
        backend
          .send(request.response(asWebSocket { (ws: WebSocket[F]) =>
            val connect: F[Unit] = for {
              _ <- Logger[F].info(
                s"[VOICE] Connected to voice gateway: $voiceGatewayUrl"
              )
              ...
              _ <- Async[F].start(
                handleVoiceEvents(ws, handshakeDeferred, helloDeferred)
              )
              _ <- helloDeferred.get.timeoutTo(
                5.seconds,
                Async[F].raiseError(
                  new RuntimeException(
                    "Timeout waiting for voice hello after 5 seconds"
                  )
                )
              )
              _ <- sendVoiceIdentify(
              ...
              _ <- Logger[F].info(
                "[VOICE] Waiting for voice connection to be ready..."
              )
              _ <- handshakeDeferred.get.rethrow.timeoutTo(
                15.seconds,
                Async[F].raiseError(
                  new RuntimeException(
                    "Timeout waiting for voice handshake after 15 seconds"
                  )
                )
              )
              ...
              _ <- Logger[F].info(
                "[VOICE] Handler blocking on shutdown signal (keeps connection alive)..."
              )
              _ <- shutdownSignal.get
              _ <- Logger[F].info(
                "[VOICE] Shutdown signal received, closing connection"
              )
            ...
            connect.handleErrorWith { error =>
              ...
              wsResult.complete(Left(error)).void >>
                Logger[F].error(
                  s"[VOICE] Voice gateway connection failed: $msg"
                )
            }
          }))
          .flatMap { response =>
            response.body match {
              case Left(error) =>
                wsResult
                  .complete(Left(new RuntimeException(error.toString)))
                  .attempt
                  .void >> Logger[F].error(s"[VOICE] WebSocket error: $error")
              case Right(_) =>
                Logger[F].info("[VOICE] WebSocket handler completed")
            }
          }
          .handleErrorWith { error =>
            wsResult.complete(Left(error)).attempt.void >>
              Logger[F].error(
                s"[VOICE] Voice WebSocket fiber error: ${errorMessage(error)}"
              )
          }
      }
      ...
      _ <- Logger[F].info(
        "[VOICE] Voice connection established and staying alive"
      )
    } yield ws
  }

  private def startVoiceHeartbeat(
      heartbeatIntervalMs: Int,
      ws: WebSocket[F]
  ): F[Unit] = {
    Logger[F].info(
      s"[VOICE] Starting voice heartbeat (interval: ${heartbeatIntervalMs}ms)"
    ) >>
      Async[F].start {
              ...
              Option
                .when(isConnected)(
                  sendHeartbeat(ws).handleErrorWith { error =>
                    ...
                    Option
                      .when(
                        errorMsg.contains("closed") || errorMsg.contains(
                          "Closed"
                        ) || errorMsg.contains("Output closed")
                      )(
                        Logger[F].warn(
                          s"[VOICE] Heartbeat stopped - WebSocket closed"
                        ) >>
                      ...
                      .getOrElse(
                        Logger[F].warn(
                          s"[VOICE] Failed to send voice heartbeat: $errorMsg"
                        )
                      )
                  }
                )
                .getOrElse(
                  Logger[F].debug(
                    s"[VOICE] Heartbeat loop exiting - connection closed"
                  ) >>
                    Async[F].raiseError[Unit](
                      new Exception("Voice connection closed")
                    )
                )
            }
          }
    ...
    def processVoiceEvent(message: String): F[Unit] = {
      (for {
          ...
          case Right(json) =>
            ...
            seqNum.update(_ => seq) >>
              (op match {
                ...
                case 31 =>
                  Logger[F].warn("[DAVE] MLS invalid commit/welcome received")
                case _ =>
                  Logger[F].debug(s"[VOICE] Unhandled voice event: op=$op")
              })
          case Left(error) =>
        ...
        handshakeDeferred.complete(Left(error)).attempt.void >>
          Logger[F].error(
            s"[VOICE] Error processing voice event: ${errorMessage(error)}"
          ) >>
        ...
        handshakeDeferred.complete(Left(error)).attempt.void >>
          (error match {
            case e: WebSocketClosed =>
              isConnectedRef.set(false) >>
                Logger[F].warn(
                  s"[VOICE] Voice WebSocket closed by Discord: ${e.frame.fold(
                      "no close frame"
                    )(f => s"code=${f.statusCode}, reason='${f.reasonText}'")}"
                )
            case _ =>
              Logger[F].error(
                s"[VOICE] Voice event loop error: ${errorMessage(error)}"
              ) >>
                Logger[F].debug(
                  s"[VOICE] Stack trace: ${error.getStackTrace.take(3).mkString("\n")}"
                )
          })
      }
  ...
  private def handleReady(json: io.circe.Json, ws: WebSocket[F]): F[Unit] = {
    ...
    val selectedMode = EncryptionMode.preferredModes
      .flatMap(preferred =>
        modes.find(_ == preferred.value).flatMap(EncryptionMode.fromString)
      )
      .headOption
    ...
    ssrcRef.set(Some(extractedSsrc)) >>
      ...
      Logger[F].info(
        s"[VOICE] Connected - encryption: ${selectedMode.value}, voiceReadySsrc=$extractedSsrc"
      ) >>
      Logger[F].debug(
        s"[VOICE] Voice ready: SSRC=$extractedSsrc, IP=$ip, Port=$port"
      ) >>
      Logger[F].debug(
        s"[VOICE] Available encryption modes: ${modes.mkString(", ")}"
      ) >>
      performIPDiscovery(ws, ip, port, extractedSsrc)
  }

  private def performIPDiscovery(
      ws: WebSocket[F],
      ip: String,
      port: Int,
      ssrc: Int
    ...
    def attemptDiscovery(
        socket: DatagramSocket,
        discoveryPacket: Array[Byte],
        attempt: Int
    ): F[Array[Byte]] = {
      val sendAndReceive =
        Async[F].blocking {
          val endpoint = new InetSocketAddress(ip, port)
          val datagram = new java.net.DatagramPacket(
            discoveryPacket,
          ...
          Logger[F].debug(
            s"[VOICE] Sent IP discovery packet (74 bytes) on attempt $attempt/$maxDiscoveryAttempts"
          ) >>
          Async[F].blocking {
            val receiveBuffer = new Array[Byte](74)
            val receivePacket =
              new java.net.DatagramPacket(receiveBuffer, receiveBuffer.length)
      ...
      sendAndReceive.attempt.flatMap {
        ...
        case Left(error)     =>
          ...
          Option
            .when(attempt < maxDiscoveryAttempts)(attempt + 1)
            .fold(
              failureLog >>
                Logger[F].error(
              ...
              failureLog >>
                Logger[F].info(
                  s"[VOICE] Retrying UDP IP discovery (attempt $nextAttempt/$maxDiscoveryAttempts)"
                ) >>
    ...
    for {
      ...
      _ <- Logger[F].debug(
        s"[VOICE] Performing UDP IP discovery to $ip:$port..."
      )
      ...
      response <- Logger[F].debug(
        s"[VOICE] Starting UDP IP discovery attempt 1/$maxDiscoveryAttempts"
      ) >>
      ...
      result <- Async[F].blocking {
        ...
        val externalIp = new String(
          ipBytes,
          0,
          Option.when(nullTerminator >= 0)(nullTerminator).getOrElse(64),
          "UTF-8"
        ).trim
      ...
      _ <- Logger[F].debug(
        s"[VOICE] IP Discovery complete: external IP=$externalIp, port=$externalPort"
      )
      _ <- sendSelectProtocol(ws, externalIp, externalPort)
    } yield ()
  }

  private def sendSelectProtocol(
      ws: WebSocket[F],
      ip: String,
      port: Int
  ): F[Unit] = {
    selectedEncryptionModeRef.get.flatMap { modeOpt =>
      ...
      ws.sendText(selectProtocol.noSpaces) >>
        Logger[F].debug(
          s"[VOICE] Sent SELECT_PROTOCOL with encryption mode: ${mode.value}"
        )
    }
  }

  private def handleSessionDescription(
      json: io.circe.Json,
      ws: WebSocket[F],
      handshakeDeferred: Deferred[F, Either[Throwable, Unit]]
  ): F[Unit] = {
    ...
    val daveProtocolVersion =
      data.get[Int]("dave_protocol_version").getOrElse(0)

    Logger[F].debug(
      s"[VOICE] Session description - mode: $mode, secret key length: ${secretKey.length}, dave=$daveProtocolVersion"
    ) >>
      daveProtocolVersionRef.set(daveProtocolVersion) >>
      Option
        .when(daveProtocolVersion > 0)(
          Logger[F].info(
            s"[DAVE] Discord selected DAVE protocol v$daveProtocolVersion; initializing MLS session"
          ) >>
            withDaveManager(_.onSelectProtocolAck(daveProtocolVersion))
              .flatMap(sendDaveActions(ws)) >>
  ...
  def startPlaying(ws: WebSocket[F]): F[Unit] = {
    ssrcRef.get.flatMap { ssrcOpt =>
      ...
      daveSessionManagerRef.get.flatMap(
        _.fold(Async[F].unit)(
          _.debugState.flatMap(state =>
            Logger[F].info(
              s"[VOICE] SPEAKING=1 correlation: voiceReadySsrc=$speakingSsrc daveState=$state"
            )
          )
        )
      ) >>
        Logger[F].info(
          s"[VOICE] Sending SPEAKING=1 payload for ssrc=$speakingSsrc..."
        ) >>
        ws.sendText(speaking.noSpaces).handleErrorWith { error =>
          Logger[F].error(
            s"[VOICE] Failed to send SPEAKING payload: ${error.getMessage}"
          ) >>
            Logger[F].error(
              s"[VOICE] WebSocket might be closed. Error type: ${error.getClass.getName}"
            ) >>
            Async[F].raiseError(error)
        } >>
    ...
    for {
      ...
      _ <- (socketOpt, ssrcOpt, ipOpt, portOpt, secretKeyOpt) match {
        case (
          ...
          val encryptionMode = encryptionModeOpt.getOrElse(
              ...
              startPlaying(ws) >>
                audioStreamer
                  .streamAudio(
                    ...
                    daveAudioEncryptor = daveEncryptor
                  )
                  .handleErrorWith { error =>
                    Logger[F].error(
                      s"[VOICE] Streaming error: ${error.getMessage}"
                    ) >>
                      stopPlaying(ws) >>
        ...
        case _ =>
          Logger[F].error(
            s"[VOICE] Voice connection not properly initialized"
          ) >>
            Logger[F].error(
  ...
  private def handleDavePrepareTransition(
      json: io.circe.Json,
      ws: WebSocket[F]
  ): F[Unit] = {
    val transitionId =
      json.hcursor.downField("d").get[Int]("transition_id").getOrElse(0)
    val protocolVersion =
      json.hcursor.downField("d").get[Int]("protocol_version").getOrElse(0)
    Logger[F].info(
      s"[DAVE] Prepare transition: id=$transitionId protocol=$protocolVersion"
    ) >>
      withDaveManager(_.onPrepareTransition(transitionId, protocolVersion))
        .flatMap(sendDaveActions(ws)) >>
  ...
  private def handleDaveClientsConnect(json: io.circe.Json): F[Unit] = {
    val userIds =
      json.hcursor.downField("d").get[List[String]]("user_ids").getOrElse(Nil)
    withDaveManager(_.addUsers(userIds))
      .flatMap(sendDaveActionsFromActiveSocket) >>
      Logger[F].info(
        s"[DAVE] Connected user_ids count=${userIds.size} values=${userIds.map(shortHash).mkString("[", ",", "]")}"
      ) >>
      ...
      withDaveManager { manager =>
        manager.onExecuteTransition(transitionId) >>
          manager.debugState.flatMap(state =>
            Logger[F].info(
              s"[DAVE] State after execute transition $transitionId: $state"
            )
          )
      }
  }

  private def handleDavePrepareEpoch(
      json: io.circe.Json,
      ws: WebSocket[F]
  ): F[Unit] = {
    ...
    Logger[F].info(
      s"[DAVE] Prepare epoch: transition_id=$transitionId epoch=$epoch protocol=$protocolVersion"
    ) >>
      daveProtocolVersionRef.set(protocolVersion) >>
      withDaveManager(
        _.onPrepareEpoch(transitionId, epoch.toLong, protocolVersion)
      ).flatMap(sendDaveActions(ws)) >>
      logDaveState(s"prepare epoch $epoch transition $transitionId")
  }

  private def handleDaveBinaryMessage(
      payload: Array[Byte],
      finalFragment: Boolean
  ): F[Unit] =
    Logger[F].info(
      s"[DAVE] Binary frame ${describeDaveBinaryEnvelope(payload, finalFragment)}"
    ) >> (
      DaveSupport.GatewayBinaryCodec.decode(payload) match {
        case Right(message: DaveSupport.MlsExternalSenderPackage) =>
          Logger[F].info(
            s"[DAVE] Received MLS external sender package seq=${message.sequenceNumber}"
          ) >>
            withDaveManager(_.onExternalSenderPackage(payload.drop(3)))
              .flatTap(logDaveActions("external sender package"))
              .flatMap(sendDaveActionsFromActiveSocket)
        case Right(message: DaveSupport.MlsProposals) =>
          Logger[F].info(
            s"[DAVE] Received MLS proposals seq=${message.sequenceNumber}, op=${message.operationType}, payload=${message.payload.length} bytes"
          ) >>
            withDaveManager(_.onMlsProposals(payload.drop(3)))
              .flatTap(logDaveActions("proposal processing"))
              .flatMap(sendDaveActionsFromActiveSocket)
        case Right(message: DaveSupport.MlsAnnounceCommitTransition) =>
          Logger[F].info(
            s"[DAVE] Received MLS commit transition seq=${message.sequenceNumber}, transition=${message.transitionId}, commit=${message.commitMessage.length} bytes"
          ) >>
            withDaveManager(
              _.onMlsCommitTransition(
        ...
        case Right(message: DaveSupport.MlsWelcome) =>
          Logger[F].info(
            s"[DAVE] Received MLS welcome seq=${message.sequenceNumber}, transition=${message.transitionId}, welcome=${message.welcomeMessage.length} bytes"
          ) >>
            withDaveManager(
              _.onMlsWelcome(message.transitionId, message.welcomeMessage)
            ).flatMap(sendDaveActionsFromActiveSocket)
        case Right(message) =>
          Logger[F].debug(s"[DAVE] Ignoring ${message.getClass.getSimpleName}")
        case Left(error) =>
          Logger[F].warn(
            s"[DAVE] Failed to decode binary voice gateway message (${payload.length} bytes): $error"
          )
      }
    )
  ...
  private def withDaveManager[A](f: DaveSessionManager[F] => F[A]): F[A] =
    daveSessionManagerRef.get.flatMap {
      ...
      case None          =>
        Async[F].raiseError(
          new RuntimeException("DAVE session manager is not initialized")
        )
    }

  private def sendDaveActionsFromActiveSocket(
      actions: List[DaveGatewayAction]
  ): F[Unit] =
    webSocketRef.get.flatMap {
      ...
      case None                    =>
        Async[F].raiseError(
          new RuntimeException(
            "Voice WebSocket is not available for DAVE response"
          )
        )
    }

  private def logDaveActions(
      context: String
  )(actions: List[DaveGatewayAction]): F[Unit] =
    if (actions.isEmpty) {
      daveSessionManagerRef.get.flatMap(
        _.traverse_(manager =>
          manager.debugState.flatMap(state =>
            Logger[F].warn(
              s"[DAVE] No outbound DAVE actions after $context; state: $state"
            )
          )
        )
      )
      ...
      val summary = actions
        .map {
          case DaveGatewayAction.SendMlsKeyPackage(payload) =>
            s"key_package=${payload.length}B"
          case DaveGatewayAction.SendMlsCommitWelcome(payload) =>
            s"commit_welcome=${payload.length}B"
          case DaveGatewayAction.SendReadyForTransition(transitionId) =>
            s"ready_transition=$transitionId"
          case DaveGatewayAction.SendInvalidCommitWelcome(transitionId) =>
            s"invalid_commit_welcome=$transitionId"
        }
        .mkString(", ")
      Logger[F].info(s"[DAVE] Outbound actions after $context: $summary")
    }

  private def sendDaveActions(
      ws: WebSocket[F]
  )(actions: List[DaveGatewayAction]): F[Unit] =
    actions.traverse_ {
      case DaveGatewayAction.SendMlsKeyPackage(keyPackage) =>
        logKeyPackageSummary(keyPackage) >> {
          val payload = DaveSupport.GatewayBinaryCodec.encode(
            DaveSupport.MlsKeyPackage(keyPackage)
          )
      ...
      case DaveGatewayAction.SendMlsCommitWelcome(commitWelcome) =>
        val payload = DaveSupport.GatewayBinaryCodec.encode(
          DaveSupport.MlsCommitWelcome(commitWelcome, None)
        )
        incrementDaveOutboundDiagnostics(diagnostics =>
          diagnostics.copy(commitWelcomesSent =
            diagnostics.commitWelcomesSent + 1L
          )
        ).flatMap { diagnostics =>
          Logger[F].info(
      ...
      case DaveGatewayAction.SendReadyForTransition(transitionId) =>
        ...
        ws.sendText(msg.noSpaces) >>
          incrementDaveOutboundDiagnostics(diagnostics =>
            diagnostics.copy(readyTransitionsSent =
              diagnostics.readyTransitionsSent + 1L
            )
          ).flatMap { diagnostics =>
            Logger[F].info(
              s"[DAVE] Sent DAVE_TRANSITION_READY #${diagnostics.readyTransitionsSent} transition_id=$transitionId"
            )
          }
      ...
      case DaveGatewayAction.SendInvalidCommitWelcome(transitionId) =>
        ...
        ws.sendText(msg.noSpaces) >>
          incrementDaveOutboundDiagnostics(diagnostics =>
            diagnostics.copy(invalidCommitWelcomesSent =
              diagnostics.invalidCommitWelcomesSent + 1L
            )
          ).flatMap { diagnostics =>
            Logger[F].warn(
              s"[DAVE] Sent MLS_INVALID_COMMIT_WELCOME #${diagnostics.invalidCommitWelcomesSent} transition_id=$transitionId"
            )
          }
    }
  ...
  private def logDaveState(context: String): F[Unit] =
    daveSessionManagerRef.get.flatMap(
      _.fold(Async[F].unit)(manager =>
        manager.debugState.flatMap(state =>
          Logger[F].info(s"[DAVE] State after $context: $state")
        )
      )
    )

  private def daveAudioEncryptor(
      ssrc: Int
  ): F[Option[Array[Byte] => F[Array[Byte]]]] =
        ...
        manager.isMediaReady.flatMap {
          case true =>
            manager.assignAudioSsrc(ssrc) >>
              manager.debugState.flatMap(state =>
                Logger[F].info(
                  s"[DAVE] Bound media encryptor to ssrc=$ssrc; state: $state"
                )
              ) >>
              Async[F].pure(
                Some((payload: Array[Byte]) =>
                  manager.encryptAudio(ssrc, payload)
                )
              )
          case false =>
            manager.debugState.flatMap { state =>
              val message =
                s"DAVE negotiated protocol v$version but media ratchet is unavailable for SSRC $ssrc: $state"
              Logger[F].error(s"[DAVE] $message") >> Async[F]
                .raiseError[Option[Array[Byte] => F[Array[Byte]]]](
  ...
  private def awaitDaveMediaReady(
    ...
    manager.isMediaReady.flatMap {
      case true =>
        manager.debugState.flatMap(state =>
          Logger[F].info(
            s"[DAVE] MLS media ratchet is ready; starting playback; state: $state"
          )
        )
      case false =>
        Option
          .when(remaining > Duration.Zero)(
            remaining - daveMediaReadyPollInterval
          )
          .fold {
            manager.debugState.flatMap { state =>
              val message =
                s"DAVE negotiated protocol but media ratchet remained unavailable after ${daveMediaReadyTimeout.toSeconds} seconds; state: $state"
              Logger[F].error(s"[DAVE] $message") >> Async[F].raiseError[Unit](
                new RuntimeException(message)
              )
            }
          } { nextRemaining =>
            Option
              .when(!waitLogged)(
                manager.debugState.flatMap(state =>
                  Logger[F].warn(
                    s"[DAVE] MLS media ratchet pending after op25/op26; waiting briefly for op27/op29/op30/op22 before playback; state: $state"
                  )
                )
              )
              .getOrElse(Async[F].unit) >>
  ...
  private def describeDaveBinaryEnvelope(
      payload: Array[Byte],
      finalFragment: Boolean
  ): String = {
    val opcodeOpt = payload.headOption.map(_ & 0xff).flatMap { firstByte =>
      Option
        .when(
          firstByte == DaveSupport.OpMlsKeyPackage || firstByte == DaveSupport.OpMlsCommitWelcome
        )(firstByte)
    ...
    val seq = opcodeOpt.fold("n/a") { opcode =>
      Option
        .when(
          opcode == DaveSupport.OpMlsKeyPackage || opcode == DaveSupport.OpMlsCommitWelcome
        )("n/a")
        .getOrElse(
          Option
    ...
    parse(message) match {
      case Right(json) =>
        ...
        val detail = op match {
          ...
          case 11 =>
            val userCount = cursor
              .downField("d")
              .get[List[String]]("user_ids")
              .fold(_ => 0, _.size)
  ...
  private def logKeyPackageSummary(keyPackage: Array[Byte]): F[Unit] =
    MlsMessages.parseKeyPackageMessage(keyPackage) match {
      case Right(parsed) =>
        ...
        val lifetimeNotBeforeZero =
          parsed.leafNode.lifetimeNotBefore.contains(0L)
        val lifetimeNotAfterMax = parsed.leafNode.lifetimeNotAfter.contains(-1L)
        val leafSignatureValid =
          MlsMessages.verifyLeafNodeSignature(parsed.leafNode)
        val keyPackageSignatureValid =
          MlsMessages.verifyKeyPackageSignature(parsed)
        Logger[F].info(
      ...
      case Left(error) =>
        Logger[F].warn(
          s"[DAVE] MLS key package summary unavailable: ${error.message}, length=${keyPackage.length}B"
        )
    }
  ...
  def closeVoiceConnection(reason: String = "unspecified"): F[Unit] = {
    shutdownSignalRef.get.flatMap {
      _.fold(
        ...
        Logger[F].info(
          s"[VOICE] Sending shutdown signal to close voice connection reason=$reason"
        ) >>
          isConnectedRef.set(false) >>
          udpSocketRef
            .getAndSet(None)
            .flatMap(
              _.traverse_(socket => Async[F].blocking(socket.close()))
            ) >>
          daveSessionManagerRef.get.flatMap(
            _.traverse_(manager => Async[F].blocking(manager.close()))
          ) >>
    ...
    for {
      ...
      _ <- (socketOpt, ssrcOpt, ipOpt, portOpt, secretKeyOpt) match {
        case (
          ...
          Logger[F].info(
            s"[VOICE] Streaming audio from position ${startPosition}s..."
          ) >>
            Logger[F].debug("[VOICE] Stream URL redacted") >>
            daveAudioEncryptor(ssrcValue).flatMap { daveEncryptor =>
              startPlaying(ws) >>
                audioStreamer
                  .streamAudioFromPosition(
                    streamUrl,
                    ws,
                    socket,
                    ssrcValue,
                  ...
                  .handleErrorWith { error =>
                    Logger[F].error(
                      ...
                      closeVoiceConnection(
                        s"audio stream error from position ${startPosition}s"
                      )
                  } >>
                stopPlaying(ws) >>
                Logger[F].info(s"[VOICE] Playback completed") >>
                closeVoiceConnection(
                  s"audio stream completed from position ${startPosition}s"
                )
            }
        case _ =>
