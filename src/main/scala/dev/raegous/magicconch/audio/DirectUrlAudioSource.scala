...
class DirectUrlAudioSource[F[_]: Async](using logger: Logger[F])
  ...
  override def extractTrackInfo(url: String): F[Option[AudioTrackInfo]] =
    Option
      .when(canHandle(url))(
        for {
          _ <- logger.info(
            s"[DirectUrlAudioSource] Extracting info for direct audio URL"
          )
  ...
  override def getStreamUrl(url: String): F[Option[String]] =
    Option
      .when(canHandle(url))(url)
      .traverse(u =>
        logger
          .info(s"[DirectUrlAudioSource] Using direct URL for streaming")
          .as(u)
      )
