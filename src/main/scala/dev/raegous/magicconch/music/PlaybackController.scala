...
class PlaybackController[F[_]: Async](
    ...
    Logger[F].debug(s"[PLAYBACK] Starting playback for guild $guildId") >>
      queueManager.getQueue(guildId).flatMap { queue =>
        queue.currentTrack.fold(
          Logger[F]
            .warn(s"[PLAYBACK] No current track to play - queue might be empty")
        ) { track =>
          Logger[F].info(s"[PLAYBACK] Playing: ${track.title}") >>
            trackExtractor
              .getStreamUrl(track.url)
              .flatMap(
                _.fold(
  ...
  def startPlayingCurrentWithPosition(
      guildId: String,
      startPosition: Int
  ): F[Unit] =
    queueManager.getQueue(guildId).flatMap { queue =>
      queue.currentTrack.fold(
        ...
        trackExtractor
          ...
          .flatMap(
            _.fold(
              Logger[F]
                .error(s"[PLAYBACK] Failed to extract stream URL for resume") >>
                queueManager.setCurrentTrack(guildId, None)
            ) { streamUrl =>
              activeVoiceConnections.get.flatMap(
                _.get(guildId).fold(
  ...
  def pausePlayback(guildId: String): F[Unit] =
    queueManager.getQueue(guildId).flatMap { queue =>
      queue.currentTrack.filter(_ => !queue.isPaused).fold(Async[F].unit) {
        track =>
          for {
    ...
    activePlaybackFibers.get.flatMap { fibers =>
      fibers
        ...
        .fold(
          ...
          Logger[F].info(
            s"[STOP] Cancelling playback fiber for guild $guildId"
          ) >>
            fiber.cancel >>
  ...
  private def playNextTrackOrLeave(guildId: String): F[Unit] =
    queueManager.getQueue(guildId).flatMap { queue =>
      Option
        .when(queue.tracks.nonEmpty)(playNextTrack(guildId))
        .getOrElse(
