 ...
 * This ensures consistent player controls across all commands and interactions.
 */
object PlayerControls {
   ...
   * Returns action rows with playback control buttons if a track is playing.
   *
   * Controls include:
   ...
   */
  def createPlayerControls(guildId: String, queue: MusicQueue): Option[List[MessageComponent]] = {
    queue.currentTrack.map { _ =>
      ...
      val pauseResumeButton = MessageComponent(
        `type` = 2, // Button
        style = Some(if (queue.isPaused) 3 else 1), // Success (green) if paused, Primary (blue) if playing
        label = Some(if (queue.isPaused) "▶️ Resume" else "⏸️ Pause"),
        custom_id = Some(s"player_${if (queue.isPaused) "resume" else "pause"}_$guildId")
      )
