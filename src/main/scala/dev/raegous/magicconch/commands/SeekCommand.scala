...
class SeekCommand[F[_]: Async](voiceManager: VoiceManager[F])(using Logger[F]) extends Command[F] {
  ...
  def execute(context: CommandContext[F]): F[CommandResult] = {
    context.args.get("position") match {
      case Some(positionStr) =>
        parsePosition(positionStr) match {
          case Some(seconds) if seconds >= 0 =>
            for {
              queue <- voiceManager.getQueue(context.guildId)
              result <- queue.currentTrack match {
                case Some(track) =>
                  ...
                  if (seconds > maxDuration) {
                    ...
                    voiceManager.seek(context.guildId, seconds) >>
                    Async[F].pure(CommandResult(
