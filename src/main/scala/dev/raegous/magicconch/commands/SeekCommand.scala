...
class SeekCommand[F[_]: Async](voiceManager: VoiceManager[F])(using Logger[F])
    extends Command[F] {
  val name = "seek"
  ...
  val arguments = List(
    CommandArgument(
      "position",
      "Position (e.g., 90, 1:30, or 1:23:45)",
      required = true
    )
  )
  ...
  /** Parse position string to seconds. Supports:
    *   - "123" -> 123 seconds
