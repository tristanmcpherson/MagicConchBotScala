...
import dev.raegous.magicconch.DiscordModels.given
          ...
          response = results match {
            ...
            case searchResults =>
              ...
              val embed = MessageEmbed(
                ...
                fields = Some(searchResults.zipWithIndex.map { case (result, index) =>
                  EmbedField(
              ...
              // Create action row with buttons (max 5 buttons per row)
              val buttons = searchResults.zipWithIndex.map { case (result, index) =>
                MessageComponent(
                  `type` = 2, // Button
                  style = Some(1), // Primary (blue)
                  label = Some(s"${index + 1}"),
                  custom_id = Some(s"search_select_${context.userId}_$index")
                )
              }
