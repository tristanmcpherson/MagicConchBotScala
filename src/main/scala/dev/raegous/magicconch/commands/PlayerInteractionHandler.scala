...
class PlayerInteractionHandler[F[_]: Async](
  ...
  override def handle(
    ...
    customId match {
      case id if id.startsWith("volume_") =>
        handleVolumeControl(id, interaction)
      case id
          if id
            .startsWith("player_pause_") || id.startsWith("player_resume_") =>
        handlePlayerPauseResume(id, interaction)
      case id if id.startsWith("player_stop_") =>
        handlePlayerStop(id, interaction, wsOpt)
      case id if id.startsWith("player_skip_") =>
        handlePlayerSkip(id, interaction)
  ...
  private def handlePlayerPauseResume(
    ...
    parseCustomId(customId, 3).fold(
      Logger[F].error(
        s"[PLAYER] Invalid player pause/resume custom_id format: $customId"
      )
    ) { parts =>
      val action = parts(1)
      val guildId = parts(2)
    ...
    val response = InteractionResponse(
      ...
      data = Some(
        InteractionResponseData(
          content = Some(
            Option
              .when(isPaused)("Playback paused")
              .getOrElse("Playback resumed")
          ),
        ...
        row.copy(components = row.components.map(_.map { button =>
          Option
            .when(
              ...
              button.copy(
                label =
                  Some(Option.when(isPaused)("Resume").getOrElse("Pause")),
                custom_id = Some(
                  s"player_${Option.when(isPaused)("resume").getOrElse("pause")}_$guildId"
                ),
  ...
  private def handlePlayerStop(
    ...
    parseCustomId(customId, 3).fold(
      Logger[F].error(
        s"[PLAYER] Invalid player stop custom_id format: $customId"
      )
    ) { parts =>
      val guildId = parts(2)
      ...
      for {
        ...
        _ <- wsOpt
          ...
          .flatTap {
            case None =>
              Logger[F].warn(
                "[PLAYER] No gateway WebSocket available to leave voice channel"
              )
            case _ => Async[F].unit
          }
  ...
  private def handlePlayerSkip(
    ...
    parseCustomId(customId, 3).fold(
      Logger[F].error(
        s"[PLAYER] Invalid player skip custom_id format: $customId"
      )
    ) { parts =>
      val guildId = parts(2)
      ...
      for {
        ...
        response = InteractionResponse(
          ...
          data = Some(
            InteractionResponseData(
              content = Some(
                newQueue.currentTrack.fold("Skipped! Queue is now empty")(
                  track => s"Skipped! Now playing: **${track.title}**"
                )
              ),
              components =
                newQueue.currentTrack.fold(Some(List.empty[MessageComponent]))(
