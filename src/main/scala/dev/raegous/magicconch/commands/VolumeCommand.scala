...
class VolumeCommand[F[_]: Async](guildSettings: GuildSettingsManager[F])(using
    Logger[F]
) extends Command[F] {
  ...
  private def showVolumeControls(
  ...
  private def buildVolumeControlResult(
      currentPercent: Int,
      userId: String
  ): CommandResult = {
    ...
    val presetRow =
      MessageComponent(`type` = 1, components = Some(presetButtons))
    val adjustRow =
      MessageComponent(`type` = 1, components = Some(adjustButtons))
  ...
  private def createPresetButtons(userId: String): List[MessageComponent] =
    List(
      MessageComponent(
        `type` = 2,
        style = Some(4),
        label = Some("Mute"),
        custom_id = Some(s"volume_${userId}_0")
      ),
      MessageComponent(
        `type` = 2,
        style = Some(2),
        label = Some("50%"),
        custom_id = Some(s"volume_${userId}_50")
      ),
      MessageComponent(
        `type` = 2,
        style = Some(1),
        label = Some("100%"),
        custom_id = Some(s"volume_${userId}_100")
      ),
      MessageComponent(
        `type` = 2,
        style = Some(2),
        label = Some("150%"),
        custom_id = Some(s"volume_${userId}_150")
      ),
      MessageComponent(
        `type` = 2,
        style = Some(3),
        label = Some("200%"),
        custom_id = Some(s"volume_${userId}_200")
      )
    )

  private def createAdjustButtons(userId: String): List[MessageComponent] =
    List(
      MessageComponent(
        `type` = 2,
        style = Some(2),
        label = Some("-10%"),
        custom_id = Some(s"volume_${userId}_dec10")
      ),
      MessageComponent(
        `type` = 2,
        style = Some(2),
        label = Some("+10%"),
        custom_id = Some(s"volume_${userId}_inc10")
      )
    )
}
