...
class JoinCommand[F[_]: Async](voiceManager: VoiceManager[F])(using Logger[F])
    extends Command[F] {
  val name = "join"
  ...
  def execute(context: CommandContext[F]): F[CommandResult] = {
    val result = for {
      ...
      _ <- OptionT.liftF(
        Logger[F].info(
          s"[JOIN] Looking up voice channel for user ${context.userId} (username: ${context.username})"
        )
      )
      channelId <- OptionT(voiceManager.getUserVoiceChannel(context.userId))
      _ <- OptionT.liftF(
        Logger[F].info(
          s"[JOIN] ✓ User ${context.userId} is in voice channel $channelId, joining..."
        )
      )
      _ <- OptionT.liftF(
  ...
  private def handleJoinFailure(
    ...
    context.gatewayWs.fold(
      ...
      Logger[F].warn(
        s"[JOIN] No voice channel found for user ${context.userId}. This usually means:"
      ) >>
        Logger[F].warn(s"[JOIN]   1. User is not in a voice channel") >>
        Logger[F].warn(
          s"[JOIN]   2. Bot doesn't have 'View Channels' permission"
        ) >>
        Logger[F].warn(
          s"[JOIN]   3. GUILD_VOICE_STATES intent not properly configured"
        ) >>
        Async[F].pure(
          CommandResult(
