...
/** Handler for Discord component interactions (buttons, select menus, etc.)
  *
  * Component interactions are triggered when users interact with message
  * components like buttons or select menus. Each handler is responsible for:
...
trait ComponentInteractionHandler[F[_]] {
  ...
  /** Check if this handler can process the given custom_id
    *
    * @param customId
    *   The custom_id from the component interaction
    * @return
    *   true if this handler should process this interaction
    */
  ...
  /** Handle the component interaction
    ...
    *   The custom_id from the component interaction
    * @param interaction
    *   The full interaction object from Discord
    * @param wsOpt
