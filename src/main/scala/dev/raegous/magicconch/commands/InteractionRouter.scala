...
class InteractionRouter[F[_]: Async](
    ...
    interaction.data.flatMap(_.custom_id) match {
      case Some(customId) =>
        handlers.find(_.canHandle(customId)) match {
          case Some(handler) =>
            Logger[F].debug(s"[INTERACTION] Routing $customId to ${handler.getClass.getSimpleName}") >>
            handler.handle(customId, interaction, wsOpt)
          case None =>
            Logger[F].warn(s"[INTERACTION] No handler found for custom_id: $customId")
        }
      case None =>
        Logger[F].warn("[INTERACTION] Message component interaction missing custom_id")
    }
  }
   ...
   */
  def withHandler(handler: ComponentInteractionHandler[F]): InteractionRouter[F] = {
