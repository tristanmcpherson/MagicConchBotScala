...
class SkipCommand[F[_]: Async](voiceManager: VoiceManager[F])(using Logger[F])
    extends Command[F] {
  val name = "skip"
    ...
    voiceManager.playNext(context.guildId).flatMap {
      case Some(nextTrack) =>
        voiceManager.startPlayingCurrent(context.guildId) >>
          Async[F].pure(
            CommandResult(s"⏭️ Skipped! Now playing: **${nextTrack.title}**")
          )
      case None =>
        Async[F].pure(
          CommandResult("❌ No more tracks in queue!", isError = true)
        )
    }
  }
}
