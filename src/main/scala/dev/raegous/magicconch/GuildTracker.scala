...
/**
 * Tracks guilds (Discord servers) that the bot is a member of.
 * Used by the dashboard to display available servers.
 */
object GuildTracker {
  def make[F[_]: Async](using Logger[F]): Resource[F, GuildTracker[F]] = {
    Resource.eval {
      Ref.of[F, Map[String, GuildTracker.TrackedGuild]](Map.empty).map { guildsRef =>
