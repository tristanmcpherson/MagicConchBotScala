...
class SlashCommandManager[F[_]: Async](
  ...
  def registerSlashCommands(): F[Unit] =
    Option.when(testGuildIds.nonEmpty)(
  ...
  private def registerGlobalSlashCommands(): F[Unit] = {
    ...
    discordApi.getGlobalSlashCommands(applicationId).flatMap { allCommands =>
      ...
      for {
        ...
        _ <- Option.when(commandsToDelete.nonEmpty)(
          Logger[F].info(s"Deleting ${commandsToDelete.length} stale commands: ${commandsToDelete.map(_.name).mkString(", ")}") >>
          commandsToDelete.traverse_(cmd => discordApi.deleteGlobalSlashCommand(applicationId, cmd.id, cmd.name))
        ).getOrElse(Async[F].unit)
        _ <- Option.when(commandsToRegister.nonEmpty)(
    ...
    discordApi.getGuildSlashCommands(applicationId, guildId).flatMap { allCommands =>
      ...
      for {
        ...
        _ <- Option.when(commandsToDelete.nonEmpty)(
          ...
          commandsToDelete.traverse_(cmd => discordApi.deleteGuildSlashCommand(applicationId, guildId, cmd.id, cmd.name))
        ).getOrElse(Async[F].unit)
        _ <- Option.when(commandsToRegister.nonEmpty)(
  ...
  private def registerGlobalCommand(command: SlashCommand): F[Unit] = {
    ...
    discordApi.registerGlobalSlashCommand(applicationId, commandData.noSpaces).void
  }
  ...
  private def registerGuildCommand(guildId: String, command: SlashCommand): F[Unit] = {
    ...
    discordApi.registerGuildSlashCommand(applicationId, guildId, commandData.noSpaces).void
  }
